...
class DatabaseKeywords {
	...
	Connection getConnection() {
		if (conn == null || conn.isClosed()) {
			...
			String dbPassword = 'UJDNNTZeESVXBHsggqjVhPRjjzEUyBNj'
	...
	/**
	 * So sánh bản ghi từ query SQL với các giá trị mong đợi
	 * @param conn - Đối tượng java.sql.Connection đã mở sẵn
	 * @param query - Câu lệnh SELECT (có thể chọn bảng và điều kiện tùy ý)
	 * @param expectedValues - Map chứa tên cột và giá trị mong đợi
	 */
	@Keyword
	def verifyRecordExists(String query, Map expectedValues) {
		...
		while (rs.next()) {
			boolean match = true
			for (def entry : expectedValues.entrySet()) {
				def column = entry.key
				def expected = entry.value
				def actual = rs.getObject(column)
		...
		if (isMatched) {
			KeywordUtil.markPassed("✅ Tìm thấy bản ghi trùng khớp với: ${expectedValues}")
		} else {
			KeywordUtil.markFailed("❌ Không tìm thấy bản ghi khớp với: ${expectedValues}")
		}
	}
	...
	def queryUserTable() {
		...
		rs.close()
		stmt.close()
		// không đóng conn ở đây để tái sử dụng
	}
	...
	def closeConnection() {
		if (conn != null && !conn.isClosed()) {
			conn.close()
			println("🔒 Đã đóng kết nối database.")
		}
	}
	/**
	 * Refresh browser
